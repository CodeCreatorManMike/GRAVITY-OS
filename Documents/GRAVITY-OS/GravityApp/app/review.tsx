...
export default function ReviewScreen() {
  ...
  const startReview = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setMessages([{ id: '0', role: 'ai', text: err.detail ?? 'Could not start review.' }]);
        return;
      }
  ...
  const sendMessage = async () => {
    if (!input.trim() || loading || !reviewId) return;
    const userText = input.trim();
    setInput('');
    setMessages(prev => [...prev, { id: Date.now().toString(), role: 'user', text: userText }]);
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/review/message`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: userText, review_id: reviewId }),
      });
      const data = await res.json();
      ...
      setMessages(prev => [...prev, { id: Date.now().toString(), role: 'ai', text: 'Something went wrong.' }]);
    } finally {
      setLoading(false);
  ...
  return (
    ...
    >
      <Text style={[styles.header, { color: INK }]}>CYCLE REVIEW</Text>
      <FlatList
        ref={flatListRef}
        data={messages}
        keyExtractor={m => m.id}
